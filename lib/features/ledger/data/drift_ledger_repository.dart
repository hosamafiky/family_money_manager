...
final class DriftLedgerRepository implements LedgerRepository {
    ...
    await _db.transaction(() async {
      ...
      await _insertContext(
        OperationContextsCompanion.insert(
          ...
          recurringNote: params.isRecurring ? const Value('recurring_marker_not_scheduled') : const Value.absent(),
          categoryCode: Value(params.categoryCode),
  ...
  @override
  Future<IdempotentOperationResult> recordExpense(RecordExpenseParams params, {ChildWithdrawalAuditParams? auditParams}) async {
    final account = await _requireAccount(params.sourceAccountId, params.householdId);
    _checkProtectedWithdrawal(account: account, auditParams: auditParams, expectedOperationId: params.operationId, expectedHouseholdId: params.householdId);
    await _checkSufficientBalance(params.sourceAccountId, params.householdId, params.amountMinorUnits);

    final now = DateTime.now().toUtc().toIso8601String();
    final entryType = account.isChildProtectedFund ? LedgerEntryType.childFundWithdrawal : LedgerEntryType.expense;
    late IdempotentOperationResult result;
    ...
    await _db.transaction(() async {
      ...
      await _insertOp(
        OperationsCompanion.insert(
          id: params.operationId,
          householdId: params.householdId,
          type: account.isChildProtectedFund ? OperationType.childFundWithdrawal.code : OperationType.expense.code,
          effectiveDate: params.effectiveDate,
          recordedAt: now,
          totalAmountMinorUnits: params.amountMinorUnits,
          currencyCode: Value(params.currencyCode),
  ...
  @override
  Future<IdempotentOperationResult> executeTransfer(ExecuteTransferParams params, {ChildWithdrawalAuditParams? auditParams}) async {
    ...
    if (source.currencyCode != destination.currencyCode) {
      throw CurrencyMismatchTransferError(sourceCode: source.currencyCode, destinationCode: destination.currencyCode);
    }
    ...
    _checkProtectedWithdrawal(account: source, auditParams: auditParams, expectedOperationId: params.operationId, expectedHouseholdId: params.householdId);
    await _checkSufficientBalance(params.sourceAccountId, params.householdId, params.amountMinorUnits);
  ...
  Future<IdempotentOperationResult> recordOpeningBalance(RecordOpeningBalanceParams params) async {
    ...
    final existing = await findOperation(operationId: params.operationId, householdId: params.householdId);
    ...
    await _db.transaction(() async {
      ...
      await _insertContext(
        OperationContextsCompanion.insert(operationId: params.operationId, householdId: params.householdId, note: Value(params.description), createdAt: now),
      );
  ...
  @override
  Future<IdempotentOperationResult> recordAdjustment(RecordAdjustmentParams params, {ChildWithdrawalAuditParams? auditParams}) async {
    ...
    await _db.transaction(() async {
      ...
      await _insertEntry(
        LedgerEntriesCompanion.insert(
          ...
          currencyCode: Value(params.currencyCode),
          entryType: params.isCredit ? LedgerEntryType.adjustmentCredit.code : LedgerEntryType.adjustmentDebit.code,
          effectiveDate: params.effectiveDate,
          recordedAt: now,
          createdBy: params.createdBy,
        ),
      ...
      await _insertContext(
        OperationContextsCompanion.insert(operationId: params.operationId, householdId: params.householdId, note: Value(params.reason), createdAt: now),
      );
  ...
  @override
  Future<IdempotentOperationResult> reverseOperation(ReverseOperationParams params, {ChildWithdrawalAuditParams? auditParams}) async {
    ...
    final existingReversal = await findOperation(operationId: params.reversalOperationId, householdId: params.householdId);
    if (existingReversal != null) {
      return IdempotentOperationResult.alreadyExists;
    }

    final original = await findOperation(operationId: params.originalOperationId, householdId: params.householdId);
    if (original == null) {
    ...
    final originalEntries = await (_db.select(
      _db.ledgerEntries,
    )..where((t) => t.operationId.equals(params.originalOperationId) & t.householdId.equals(params.householdId))).get();
      ...
      if (entry.direction == LedgerDirection.credit.code) {
        ...
        if (account.requiresWithdrawalAudit) {
          ...
          if (auditParams.operationId != params.reversalOperationId) {
            throw AuditOperationMismatchError(auditOperationId: auditParams.operationId, expectedOperationId: params.reversalOperationId);
          }
          if (auditParams.accountId != entry.accountId) {
            throw AuditAccountMismatchError(auditAccountId: auditParams.accountId, expectedAccountId: entry.accountId);
          }
    ...
    await _db.transaction(() async {
      ...
      await _insertOp(
        OperationsCompanion.insert(
          ...
          createdAt: now,
          updatedAt: now,
          description: Value(params.reason ?? 'Reversal of operation ${params.originalOperationId}'),
      ...
      for (final e in originalEntries) {
        final oppositeDirection = e.direction == LedgerDirection.credit.code ? LedgerDirection.debit.code : LedgerDirection.credit.code;
        final reversalEntryType = e.direction == LedgerDirection.credit.code ? LedgerEntryType.reversalDebit.code : LedgerEntryType.reversalCredit.code;
      ...
      await (_db.update(_db.operations)..where((t) => t.id.equals(params.originalOperationId) & t.householdId.equals(params.householdId))).write(
        OperationsCompanion(isReversed: const Value(true), reversedBy: Value(params.reversalOperationId), updatedAt: Value(now)),
      );
  ...
  @override
  Future<List<LedgerEntry>> entriesForAccount({required String accountId, required String householdId}) async {
    final rows =
  ...
  @override
  Future<Operation?> findOperation({required String operationId, required String householdId}) async {
    final row = await (_db.select(_db.operations)..where((t) => t.id.equals(operationId) & t.householdId.equals(householdId))).getSingleOrNull();
    return row == null ? null : _toOperation(row);
  }

  @override
  Future<List<Operation>> operationsInRange({required String householdId, required String fromDate, required String toDate}) async {
    final rows =
        await (_db.select(_db.operations)
              ..where((t) => t.householdId.equals(householdId) & t.effectiveDate.isBiggerOrEqualValue(fromDate) & t.effectiveDate.isSmallerOrEqualValue(toDate))
              ..orderBy([
                (t) => OrderingTerm.asc(t.effectiveDate),
                (t) => OrderingTerm.asc(t.id), // tie-breaker for INV-012
              ]))
  ...
  Future<IdempotentOperationResult?> _checkIdempotency({required String operationId, required String householdId, required String idempotencyKey}) async {
    // Check 1: exact operation ID already exists → safe retry.
    final existingById = await (_db.select(_db.operations)..where((t) => t.id.equals(operationId) & t.householdId.equals(householdId))).getSingleOrNull();
    ...
    if (idempotencyKey != operationId) {
      final existingByKey = await (_db.select(
        _db.operations,
      )..where((t) => t.idempotencyKey.equals(idempotencyKey) & t.householdId.equals(householdId))).getSingleOrNull();
  ...
  Future<FinancialAccount> _requireAccount(String accountId, String householdId) async {
    final row = await (_db.select(_db.financialAccounts)..where((t) => t.id.equals(accountId) & t.householdId.equals(householdId))).getSingleOrNull();
    if (row == null) {
  ...
  Future<bool> _hasOpeningBalance(String accountId, String householdId) async {
    final rows = await (_db.select(
      _db.ledgerEntries,
    )..where((t) => t.accountId.equals(accountId) & t.householdId.equals(householdId) & t.entryType.equals(LedgerEntryType.openingBalance.code))).get();
    return rows.isNotEmpty;
  }

  Future<void> _checkSufficientBalance(String accountId, String householdId, int amount) async {
    final entries = await (_db.select(_db.ledgerEntries)..where((t) => t.accountId.equals(accountId) & t.householdId.equals(householdId))).get();
    ...
    if (balance < amount) {
      throw InsufficientFundsError(accountId: accountId, availableMinorUnits: balance, requestedMinorUnits: amount);
    }
  }
  ...
  void _checkProtectedWithdrawal({
    ...
    if (auditParams.operationId != expectedOperationId) {
      throw AuditOperationMismatchError(auditOperationId: auditParams.operationId, expectedOperationId: expectedOperationId);
    }
    if (auditParams.accountId != account.id) {
      throw AuditAccountMismatchError(auditAccountId: auditParams.accountId, expectedAccountId: account.id);
    }
  ...
  Operation _toOperation(DbOperation row) => Operation(
    ...
    beneficiaryRole: row.beneficiaryRole != null ? HouseholdMemberRole.fromCode(row.beneficiaryRole!) : null,
    ...
    tags: row.tags != null ? row.tags!.split(',').where((String s) => s.isNotEmpty).toList() : <String>[],
