...
final class SpendingAttributionReport {
  const SpendingAttributionReport({required this.bySpender, required this.byBeneficiary, required this.byScope});
...
final class GetSpendingAttributionReportUseCase {
  ...
  Future<AppResult<SpendingAttributionReport>> execute(FinancialReportRequest req) async {
    ...
    try {
      ...
      return AppOk(SpendingAttributionReport(bySpender: bySpender, byBeneficiary: byBeneficiary, byScope: byScope));
    } catch (_) {
