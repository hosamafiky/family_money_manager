-- True schema v18 objects from onCreate at commit 47fd59676d5a9a06ac6d4ea6f9b6ae3c256e4729
-- (Phase 6B.1 tip / last schemaVersion => 18 before Phase 6B.1.1).
-- Tables materialised separately from Drift CREATE TABLE DDL.
-- user_version observed: 18

-- index: idx_budgets_idempotency
CREATE UNIQUE INDEX idx_budgets_idempotency
      ON budgets(household_id, idempotency_key)
    ;

-- index: idx_certificate_events_certificate
CREATE INDEX idx_certificate_events_certificate
      ON certificate_events(certificate_id, created_at)
    ;

-- index: idx_certificate_events_idempotency
CREATE UNIQUE INDEX idx_certificate_events_idempotency
      ON certificate_events(household_id, idempotency_key)
      WHERE idempotency_key IS NOT NULL
    ;

-- index: idx_certificate_events_related_op
CREATE UNIQUE INDEX idx_certificate_events_related_op
      ON certificate_events(related_operation_id)
      WHERE related_operation_id IS NOT NULL
    ;

-- index: idx_certificate_revisions_certificate
CREATE INDEX idx_certificate_revisions_certificate
      ON certificate_revisions(certificate_id)
    ;

-- index: idx_financial_accounts_archived
CREATE INDEX idx_financial_accounts_archived
      ON financial_accounts(is_archived)
    ;

-- index: idx_financial_accounts_household
CREATE INDEX idx_financial_accounts_household
      ON financial_accounts(household_id)
    ;

-- index: idx_financial_accounts_idempotency
CREATE UNIQUE INDEX idx_financial_accounts_idempotency
      ON financial_accounts(household_id, idempotency_key)
      WHERE idempotency_key IS NOT NULL
    ;

-- index: idx_goal_lifecycle_hh_idem
CREATE UNIQUE INDEX idx_goal_lifecycle_hh_idem
      ON goal_lifecycle_events(household_id, idempotency_key)
      WHERE idempotency_key IS NOT NULL
    ;

-- index: idx_goal_movements_goal
CREATE INDEX idx_goal_movements_goal
      ON goal_movements(goal_id)
    ;

-- index: idx_goal_movements_one_reversal_per_original
CREATE UNIQUE INDEX idx_goal_movements_one_reversal_per_original
      ON goal_movements(reversal_of_movement_id)
      WHERE reversal_of_movement_id IS NOT NULL
    ;

-- index: idx_goal_movements_operation
CREATE INDEX idx_goal_movements_operation
      ON goal_movements(transfer_operation_id)
    ;

-- index: idx_goal_movements_unique_operation
CREATE UNIQUE INDEX idx_goal_movements_unique_operation
      ON goal_movements(transfer_operation_id)
    ;

-- index: idx_goal_revisions_goal
CREATE INDEX idx_goal_revisions_goal
      ON goal_revisions(goal_id)
    ;

-- index: idx_goals_idempotency
CREATE UNIQUE INDEX idx_goals_idempotency
      ON goals(household_id, idempotency_key)
    ;

-- index: idx_goals_reserve_account
CREATE UNIQUE INDEX idx_goals_reserve_account
      ON goals(reserve_account_id)
    ;

-- index: idx_ledger_entries_account
CREATE INDEX idx_ledger_entries_account
      ON ledger_entries(account_id)
    ;

-- index: idx_ledger_entries_effective_date
CREATE INDEX idx_ledger_entries_effective_date
      ON ledger_entries(effective_date)
    ;

-- index: idx_ledger_entries_household
CREATE INDEX idx_ledger_entries_household
      ON ledger_entries(household_id)
    ;

-- index: idx_ledger_entries_operation
CREATE INDEX idx_ledger_entries_operation
      ON ledger_entries(operation_id)
    ;

-- index: idx_ledger_idempotency
CREATE UNIQUE INDEX idx_ledger_idempotency
      ON ledger_entries(operation_id, account_id, direction, entry_type)
    ;

-- index: idx_operations_date
CREATE INDEX idx_operations_date
      ON operations(effective_date)
    ;

-- index: idx_operations_household
CREATE INDEX idx_operations_household
      ON operations(household_id)
    ;

-- index: idx_operations_idempotency_key
CREATE UNIQUE INDEX idx_operations_idempotency_key
      ON operations(household_id, idempotency_key)
      WHERE idempotency_key IS NOT NULL
    ;

-- index: idx_operations_type
CREATE INDEX idx_operations_type
      ON operations(type)
    ;

-- index: idx_savings_certificates_account
CREATE UNIQUE INDEX idx_savings_certificates_account
      ON savings_certificates(certificate_account_id)
    ;

-- index: idx_savings_certificates_household
CREATE INDEX idx_savings_certificates_household
      ON savings_certificates(household_id, lifecycle, maturity_date)
    ;

-- index: idx_savings_certificates_idempotency
CREATE UNIQUE INDEX idx_savings_certificates_idempotency
      ON savings_certificates(household_id, idempotency_key)
    ;

-- trigger: certificate_event_household_match
CREATE TRIGGER certificate_event_household_match
...
-- trigger: certificate_lifecycle_valid_transition
CREATE TRIGGER certificate_lifecycle_valid_transition
...
-- trigger: certificate_revision_household_match
CREATE TRIGGER certificate_revision_household_match
...
-- trigger: check_audit_amount
CREATE TRIGGER check_audit_amount
...
-- trigger: check_audit_reason
CREATE TRIGGER check_audit_reason
      BEFORE INSERT ON child_withdrawal_audits
...
-- trigger: check_audit_warning_shown
CREATE TRIGGER check_audit_warning_shown
      BEFORE INSERT ON child_withdrawal_audits
...
-- trigger: check_certificate_lifecycle_insert
CREATE TRIGGER check_certificate_lifecycle_insert
...
-- trigger: check_certificate_lifecycle_update
CREATE TRIGGER check_certificate_lifecycle_update
      BEFORE UPDATE OF lifecycle ON savings_certificates
      FOR EACH ROW
      WHEN NEW.lifecycle NOT IN ('active', 'redeemed', 'archived')
      BEGIN
...
-- trigger: check_goal_lifecycle_status
CREATE TRIGGER check_goal_lifecycle_status
...
-- trigger: check_goal_lifecycle_status_update
CREATE TRIGGER check_goal_lifecycle_status_update
...
-- trigger: check_ledger_entry_amount
CREATE TRIGGER check_ledger_entry_amount
...
-- trigger: check_operation_amount
CREATE TRIGGER check_operation_amount
...
-- trigger: fk_audit_operation_household
CREATE TRIGGER fk_audit_operation_household BEFORE INSERT ON child_withdrawal_audits WHEN NOT EXISTS (  SELECT 1 FROM operations   WHERE id = NEW.operation_id AND household_id = NEW.household_id) BEGIN   SELECT RAISE(ABORT, 'child_withdrawal_audits.operation_id must reference an operation in the same household'); END;

-- trigger: fk_goal_lifecycle_event_household_id
CREATE TRIGGER fk_goal_lifecycle_event_household_id
...
-- trigger: fk_ledger_entry_operation_id
CREATE TRIGGER fk_ledger_entry_operation_id BEFORE INSERT ON ledger_entries WHEN NOT EXISTS (  SELECT 1 FROM operations   WHERE id = NEW.operation_id AND household_id = NEW.household_id) BEGIN   SELECT RAISE(ABORT, 'ledger_entries.operation_id must reference an existing operations.id in the same household'); END;

-- trigger: fk_operation_context_operation_id
CREATE TRIGGER fk_operation_context_operation_id BEFORE INSERT ON operation_contexts WHEN NOT EXISTS (   SELECT 1 FROM operations WHERE id = NEW.operation_id ) BEGIN   SELECT RAISE(ABORT, 'operation_contexts.operation_id must reference an existing operations.id'); END;

-- trigger: goal_lifecycle_household_matches_goal
CREATE TRIGGER goal_lifecycle_household_matches_goal
      BEFORE INSERT ON goal_lifecycle_events
      FOR EACH ROW
      BEGIN
...
-- trigger: goal_movement_release_reason
CREATE TRIGGER goal_movement_release_reason
...
-- trigger: goal_movement_transfer_type
CREATE TRIGGER goal_movement_transfer_type
      BEFORE INSERT ON goal_movements
      FOR EACH ROW
      BEGIN
...
-- trigger: goal_revision_beneficiary_same_household
CREATE TRIGGER goal_revision_beneficiary_same_household
...
-- trigger: goal_status_valid_transition
CREATE TRIGGER goal_status_valid_transition
...
-- trigger: immutable_account_type_currency
CREATE TRIGGER immutable_account_type_currency BEFORE UPDATE ON financial_accounts WHEN OLD.type != NEW.type OR OLD.currency_code != NEW.currency_code BEGIN   SELECT RAISE(ABORT, 'Account type and currency are immutable after creation'); END;

-- trigger: no_archive_active_reserve
CREATE TRIGGER no_archive_active_reserve
...
-- trigger: no_cross_household_member
CREATE TRIGGER no_cross_household_member BEFORE INSERT ON household_members BEGIN   SELECT RAISE(ABORT, 'household_members.household_id must match household.id')   WHERE NOT EXISTS (     SELECT 1 FROM households WHERE id = NEW.household_id   ); END;

-- trigger: no_delete_certificate_events
CREATE TRIGGER no_delete_certificate_events
      BEFORE DELETE ON certificate_events
      BEGIN
        SELECT RAISE(ABORT, 'certificate_events cannot be deleted');
      END;

-- trigger: no_delete_certificate_revisions
CREATE TRIGGER no_delete_certificate_revisions
      BEFORE DELETE ON certificate_revisions
      BEGIN
        SELECT RAISE(ABORT, 'certificate_revisions cannot be deleted');
      END;

-- trigger: no_delete_certificate_with_history
CREATE TRIGGER no_delete_certificate_with_history
...
-- trigger: no_delete_child_audits
CREATE TRIGGER no_delete_child_audits
      BEFORE DELETE ON child_withdrawal_audits
      BEGIN
        SELECT RAISE(ABORT, 'Child withdrawal audits cannot be deleted');
      END;

-- trigger: no_delete_goal_lifecycle_events
CREATE TRIGGER no_delete_goal_lifecycle_events
      BEFORE DELETE ON goal_lifecycle_events
      BEGIN
        SELECT RAISE(ABORT, 'goal lifecycle events cannot be deleted');
      END;

-- trigger: no_delete_goal_movements
CREATE TRIGGER no_delete_goal_movements
      BEFORE DELETE ON goal_movements
      BEGIN
        SELECT RAISE(ABORT, 'goal_movements cannot be deleted');
      END;

-- trigger: no_delete_goal_revisions
CREATE TRIGGER no_delete_goal_revisions
      BEFORE DELETE ON goal_revisions
      BEGIN
        SELECT RAISE(ABORT, 'goal_revisions cannot be deleted');
      END;

-- trigger: no_delete_goal_with_history
CREATE TRIGGER no_delete_goal_with_history
...
-- trigger: no_delete_ledger_entries
...
-- trigger: no_delete_operation_contexts
CREATE TRIGGER no_delete_operation_contexts BEFORE DELETE ON operation_contexts BEGIN   SELECT RAISE(ABORT, 'Operation contexts cannot be deleted'); END;

-- trigger: no_delete_operations
CREATE TRIGGER no_delete_operations BEFORE DELETE ON operations BEGIN   SELECT RAISE(ABORT, 'Operations cannot be deleted; use a reversal operation instead'); END;

-- trigger: no_modify_certificate_owner_type
CREATE TRIGGER no_modify_certificate_owner_type
...
-- trigger: no_modify_certificate_protected
CREATE TRIGGER no_modify_certificate_protected
...
-- trigger: no_modify_certificate_spendable
CREATE TRIGGER no_modify_certificate_spendable
...
CREATE TRIGGER no_modify_reserve_owner_type
      BEFORE UPDATE ON financial_accounts
      FOR EACH ROW
...
-- trigger: no_modify_reserve_protected
CREATE TRIGGER no_modify_reserve_protected
      BEFORE UPDATE ON financial_accounts
      FOR EACH ROW
...
-- trigger: no_modify_reserve_spendable
CREATE TRIGGER no_modify_reserve_spendable
      BEFORE UPDATE ON financial_accounts
      FOR EACH ROW
...
-- trigger: no_retype_certificate_account
CREATE TRIGGER no_retype_certificate_account
...
-- trigger: no_retype_reserve_account
CREATE TRIGGER no_retype_reserve_account
      BEFORE UPDATE ON financial_accounts
      FOR EACH ROW
...
-- trigger: no_update_certificate_events
CREATE TRIGGER no_update_certificate_events
      BEFORE UPDATE ON certificate_events
      BEGIN
        SELECT RAISE(ABORT, 'certificate_events are immutable and cannot be updated');
      END;

-- trigger: no_update_certificate_immutable
CREATE TRIGGER no_update_certificate_immutable
...
-- trigger: no_update_operation_contexts
CREATE TRIGGER no_update_operation_contexts BEFORE UPDATE ON operation_contexts BEGIN   SELECT RAISE(ABORT, 'Operation contexts are immutable and cannot be updated'); END;

-- trigger: one_primary_user_per_household
CREATE TRIGGER one_primary_user_per_household BEFORE INSERT ON household_members WHEN NEW.role = 'primary_user' AND NEW.is_archived = 0 BEGIN   SELECT RAISE(ABORT, 'Only one active primary_user per household')   WHERE EXISTS (     SELECT 1 FROM household_members     WHERE household_id = NEW.household_id       AND role = 'primary_user'       AND is_archived = 0   ); END;

-- trigger: one_spouse_per_household
CREATE TRIGGER one_spouse_per_household BEFORE INSERT ON household_members WHEN NEW.role = 'spouse' AND NEW.is_archived = 0 BEGIN   SELECT RAISE(ABORT, 'Only one active spouse per household in V1')   WHERE EXISTS (     SELECT 1 FROM household_members     WHERE household_id = NEW.household_id       AND role = 'spouse'       AND is_archived = 0   ); END;

-- trigger: prevent_negative_account_balance
...
-- trigger: reject_unsupported_goal_status_transition
CREATE TRIGGER reject_unsupported_goal_status_transition
      BEFORE UPDATE OF status ON goals
      FOR EACH ROW
      WHEN NEW.status != OLD.status
        AND NOT (
...
-- trigger: restrict_account_classification_update
CREATE TRIGGER restrict_account_classification_update
      BEFORE UPDATE ON financial_accounts
      FOR EACH ROW
      WHEN (
...
-- trigger: restrict_child_fund_unprotect
CREATE TRIGGER restrict_child_fund_unprotect
      BEFORE UPDATE ON financial_accounts
      FOR EACH ROW
...
-- trigger: restrict_operations_update
CREATE TRIGGER restrict_operations_update BEFORE UPDATE ON operations WHEN NEW.id != OLD.id OR NEW.household_id != OLD.household_id   OR NEW.type != OLD.type OR NEW.effective_date != OLD.effective_date   OR NEW.recorded_at != OLD.recorded_at   OR NEW.total_amount_minor_units != OLD.total_amount_minor_units   OR NEW.currency_code != OLD.currency_code   OR NEW.created_by != OLD.created_by OR NEW.created_at != OLD.created_at BEGIN   SELECT RAISE(ABORT, 'Operations are append-only: only is_reversed, reversed_by, and updated_at may change'); END;

-- trigger: validate_certificate_account_on_insert
CREATE TRIGGER validate_certificate_account_on_insert
      BEFORE INSERT ON savings_certificates
      FOR EACH ROW
      BEGIN
        SELECT RAISE(ABORT,
...
-- trigger: validate_certificate_profit_event
CREATE TRIGGER validate_certificate_profit_event
      BEFORE INSERT ON certificate_events
      FOR EACH ROW
...
-- trigger: validate_certificate_purchase_event
CREATE TRIGGER validate_certificate_purchase_event
      BEFORE INSERT ON certificate_events
      FOR EACH ROW
...
-- trigger: validate_certificate_redemption_event
CREATE TRIGGER validate_certificate_redemption_event
      BEFORE INSERT ON certificate_events
      FOR EACH ROW
...
-- trigger: validate_funding_movement
CREATE TRIGGER validate_funding_movement
      BEFORE INSERT ON goal_movements
      FOR EACH ROW
...
-- trigger: validate_funding_movement_household
CREATE TRIGGER validate_funding_movement_household
      BEFORE INSERT ON goal_movements
      FOR EACH ROW
      WHEN NEW.movement_type = 'funding'
      BEGIN
...
-- trigger: validate_goal_reserve_on_insert
CREATE TRIGGER validate_goal_reserve_on_insert
      BEFORE INSERT ON goals
      BEGIN
...
-- trigger: validate_release_movement_household
CREATE TRIGGER validate_release_movement_household
      BEFORE INSERT ON goal_movements
      FOR EACH ROW
      WHEN NEW.movement_type = 'release'
      BEGIN
...
CREATE TRIGGER validate_reversal_movement_link
      BEFORE INSERT ON goal_movements
      FOR EACH ROW
