...
void main() {
  ...
  group('Transfer persistence', () {
    test('1: valid transfer → debit on source, credit on destination, context', () async {
    ...
    test('3: currency mismatch rejected (CurrencyMismatchTransferError)', () async {
      await db.customStatement(
        "INSERT INTO households (id, name, owner_user_id, created_at, updated_at) "
        "VALUES ('hh-tf-usd', 'USD HH', 'user-1', '2024-01-01', '2024-01-01')",
      );
      final egpAcc = await createAccount(suffix: 't3-egp', currencyCode: 'EGP');
    ...
    test('4: insufficient source balance rejected (InsufficientFundsError)', () async {
    ...
    test('5: archived source rejected '
        '(ArchivedAccountError from _requireAccount in DriftLedgerRepository)', () async {
    ...
    test('6: archived destination rejected '
        '(ArchivedAccountError from _requireAccount in DriftLedgerRepository)', () async {
    ...
    test('7: transfer not classified as income or expense (type = transfer)', () async {
    ...
    test('9: conflicting retry — same idempotency key, different ID → conflict', () async {
  ...
  group('Spouse-wallet scenario (7-step DB test)', () {
    test('11: 2000 funded → 1300 spent → 200 returned = 500 balance + query', () async {
      ...
      expect(summary.totalReturned, 20000); // 200.00 EGP returned via transfer
