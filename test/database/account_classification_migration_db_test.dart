...
void main() {
  ...
  test('1: fresh v6 DB has restrict_account_classification_update trigger', () async {
    ...
    expect(
      ...
      reason: 'Schema v6 must install the post-history classification lock trigger',
    );
  });
  ...
  test('3: existing accounts are preserved and triggers enforce immutability', () async {
    final db = AppDatabase.forTesting();
    addTearDown(db.close);
    ...
    final found = await accountRepo.findById(id: accId, householdId: 'hh-mig-1');
    ...
    expect(
      ...
      contains('immutable'),
      reason: 'restrict_account_classification_update must fire on existing accounts',
    );
    ...
    final renamed = await accountRepo.findById(id: accId, householdId: 'hh-mig-1');
