...
void main() {
  ...
  Future<int> countOperations(String householdId) async {
    final rows = await db
        .customSelect("SELECT id FROM operations WHERE household_id = '$householdId'")
        .get();
    return rows.length;
  }
  ...
  Future<int> countEntries(String householdId) async {
    final rows = await db
        .customSelect("SELECT id FROM ledger_entries WHERE household_id = '$householdId'")
        .get();
    return rows.length;
  }

  group('Account atomicity', () {
    test('zero opening balance → account only, no operations or entries', () async {
    ...
    test('null opening balance → account only, no operations or entries', () async {
    ...
    test('non-zero opening balance → account + exactly 1 operation + at least 1 entry', () async {
    ...
    test('same idempotency key retry → returns existing account, no extra rows', () async {
    ...
    test('invalid params (empty name) → AppValidationFailure, no DB write', () async {
