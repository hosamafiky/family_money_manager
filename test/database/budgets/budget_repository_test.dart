...
void main() {
  ...
  test(
    '3. Idempotency conflict: same key + different payload → AppDuplicateConflict',
    () async {
      final plan = plan0(
        idempotencyKey: 'same-key',
        idempotencyPayload: 'payload-A',
      );
      await budgetRepo.createBudget(plan);
  ...
  test(
    '4. Cross-household isolation: same key in different household is separate',
    () async {
  ...
  test('7. Update budget filter — stored correctly', () async {
    ...
    final found = await budgetRepo.findBudgetById(plan.id);
    expect(
      (found as AppOk<BudgetPlan?>).value!.filter.categoryCode,
      equals('groceries'),
    );
  });
  ...
  test('10. List budgets — excludes archived by default', () async {
    ...
    await budgetRepo.createBudget(
      plan0(id: 'b2', idempotencyKey: 'ik-b2', name: 'Other'),
    );
  ...
  test('11. List budgets — includes archived when flag set', () async {
    ...
    final result = await budgetRepo.listBudgets(
      householdId: _hh,
  ...
  test('12. List budgets — isolated per household', () async {
    await budgetRepo.createBudget(
      plan0(id: 'b-hh1', idempotencyKey: 'ik-hh1', householdId: _hh),
    );
    await budgetRepo.createBudget(
      plan0(id: 'b-hh2', idempotencyKey: 'ik-hh2', householdId: _hh2),
    );
  ...
  test('14. getBudgetTransactions — returns only matching currency', () async {
    ...
    final usdAcc = await createAccount(
      id: 'acc-usd',
      householdId: _hh,
      currency: 'USD',
    );
  ...
  test('19. getBudgetTransactions — beneficiary filter works', () async {
    ...
    await expense(
      ...
      1500,
      '2024-03-10',
      beneficiaryMemberId: 'member-alice',
    );
  ...
  test(
    '21. Combined filters (date + category) use intersection semantics',
    () async {
  ...
  test(
    '22. getBudgetTransactions — excludes reversed expenses (restated)',
    () async {
  ...
  test(
    '24. getBudgetTransactions — backdated expense in correct period',
    () async {
  ...
  test(
    '25. Same-period reversal: expense reversed same period → excluded',
    () async {
  ...
  test(
    '26. Cross-period reversal: original expense still excluded in its period',
    () async {
  ...
  test(
    '31. Dashboard total == report total for same period and filters',
    () async {
